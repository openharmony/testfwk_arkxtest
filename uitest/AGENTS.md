...
| 新增/改 uitest Shell 命令（`uitest <命令>`） | `server/server_main.cpp`（`main:523`、`GetParam:182`、`HELP_MSG:60`） | § 新增 Shell 命令 |
...
新增 `uitest` CLI 顶层命令（如 `dumpLayout`/`screenCap`/`uiRecord`/`uiInput` 同级）须同步修改以下 5 处：

| 步骤 | 文件（行号） | 内容 |
| --- | --- | --- |
| 1. 实现处理函数 | `server/server_main.cpp`（`UiServerMain` 类） | 新增 `static int32_t HandleXxx(int32_t argc, char *argv[])`，用 `GetParam`（`:182`）解析参数 |
| 2. 注册命令分发 | `server/server_main.cpp`（`main:523`） | 在命令 if-else 链中添加 `else if (command == "命令名") { _Exit(HandleXxx(argc, argv)); }` |
| 3. 更新帮助 | `server/server_main.cpp`（`HELP_MSG:60`） | 在帮助文本中添加命令说明行 |
| 4. 补充单元测试 | `test/`（如 `ui_action_test.cpp`） | 为新命令添加 gtest 用例，覆盖参数解析、合法/非法输入、边界 |
| 5. 回归验证 | 见 § 验证矩阵 | 至少跑一遍现有 `dumpLayout` + `uiInput` 回归，确认未破坏既有命令 |

### 约束（新增 Shell 命令必读）

- **参数解析用 `GetParam`**（`:182`）：统一走 `getopt_long`，禁止手写 argv 索引解析。
- **涉及显示器/多用户须设 `displayId`**：`dumpLayout`/`screenCap` 等命令在创建 controller 前须调用 `GetUserIdByDisplayId` + `SetActiveUser`（`server_main.cpp:228`），确保针对正确用户的显示器（见 § 多屏多用户）。
- **`-p <savePath>` 须转文件描述符**：客户端将路径转 fd，服务端通过 `write(fd, ...)` 写入，禁止在服务端用文件路径直接打开（见 `dumpLayout` 实现 `:308`）。
- **禁用 `m_` 前缀**：命令选项结构体字段命名遵循 PascalCase 无前缀（`savePath_`、`displayId_`，见 § 代码风格）。

### 子命令（如 `uiInput <子命令>`）

新增 `uiInput`/`uiRecord` 的子命令（如 `click`/`swipe` 同级）须在 `UiActionInput`（`:520`）或 `UiRecord`（`:460`）的子命令分发中添加，并更新对应子命令帮助表（见 § Shell 命令 uiInput 子命令表）。
