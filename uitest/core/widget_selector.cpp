...
namespace OHOS::uitest {
    ...
    void WidgetSelector::WriteIntoParcel(json &data) const
    {
        json selfMatcherList = json::array();
        for (auto &matcher:selfMatchers_) {
            json matcherSerialized;
            matcher.WriteIntoParcel(matcherSerialized);
            selfMatcherList.push_back(matcherSerialized);
        }
        data["self"] = selfMatcherList;

        json frontLocatorJsonList = json::array();
        for (auto &locator:frontLocators_) {
            json matcherSerialized;
            locator.WriteIntoParcel(matcherSerialized);
            frontLocatorJsonList.push_back(matcherSerialized);
        }
        data["front"] = frontLocatorJsonList;

        json rearLocatorJsonList = json::array();
        for (auto &locator:rearLocators_) {
            json matcherSerialized;
            locator.WriteIntoParcel(matcherSerialized);
            rearLocatorJsonList.push_back(matcherSerialized);
        }
        data["rear"] = rearLocatorJsonList;
    }

    void WidgetSelector::ReadFromParcel(const json &data)
    {
        json selfMatcherJsonList = data["self"];
        for (auto &selfMatcherJson:selfMatcherJsonList) {
            auto matcher = WidgetAttrMatcher("", "", EQ);
            matcher.ReadFromParcel(selfMatcherJson);
            selfMatchers_.emplace_back(matcher);
        }

        json frontLocatorJsonList = data["front"];
        for (auto &frontLocatorJson:frontLocatorJsonList) {
            auto locator = WidgetSelector();
            locator.ReadFromParcel(frontLocatorJson);
            frontLocators_.emplace_back(locator);
        }

        json rearLocatorJsonList = data["rear"];
        for (auto &rearLocatorJson:rearLocatorJsonList) {
            auto locator = WidgetSelector();
            locator.ReadFromParcel(rearLocatorJson);
            rearLocators_.emplace_back(locator);
        }
    }
}